...
static const char* host = "AnTiFa-AntonTimFalcon.azure-devices.net";
static const char* device_id = "iot-device-1";
static const char* device_key = "PLrU1ZleWnyOEEy0/mNBTrA8/qup9XcNMmQ25GihWX8=";
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  // Konvertierung des empfangenen Payloads in einen String
  ...
  // Parsing des JSON-Strings
  StaticJsonDocument<200> doc;
  DeserializationError error = deserializeJson(doc, receivedPayload);

  // Überprüfung auf Fehler beim Parsen
  if (error) {
    Serial.println("Failed to parse JSON");
    return;
  }

  // Abrufen des LightState aus dem JSON-Dokument
  int lightState = doc["lightState"];
...
void toggleLight() {
  const bool currentLightState = digitalRead(lightPin);
...
static char* getTelemetryPayload() {
  ...
  bool lightValue = digitalRead(lightPin);

  // Werte in Strings umwandeln
  char stringSensorValue[10];
  sprintf(stringSensorValue, "%d", sensorValue);

  char stringLightValue[10];
  sprintf(stringLightValue, "%d", lightValue);

  // Erstelle JSON-Zeichenfolge
  az_span temp_span = az_span_create(telemetry_payload, sizeof(telemetry_payload));
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR("{ \"SensorValue\": "));
  temp_span = az_span_copy(temp_span, az_span_create((uint8_t*)stringSensorValue, strlen(stringSensorValue)));
  temp_span = az_span_copy(temp_span, AZ_SPAN_FROM_STR(", \"LightState\": "));
  temp_span = az_span_copy(temp_span, az_span_create((uint8_t*)stringLightValue, strlen(stringLightValue)));
